...
public class RunAPI {
    public void run(GuiRunUnit gunit, JTextArea jTextArea) throws Exception {
        int i=1;
        int insertsize=0;
        String fasta=gunit.fastafile;
        String fq1=gunit.fastqfile1;
        String fq2=gunit.fastqfile2;
        String tempdir=gunit.cache+"/DIPinstemp";
        String bwa="bwa";
        String chr=gunit.chr;
        if(gunit.bwadir!=null&&gunit.bwadir.equals("")==false)
            bwa=gunit.bwadir+"/bwa";
        String seqtk="seqtk";
        if(gunit.seqtkdir!=null&&gunit.seqtkdir.equals("")==false)
            seqtk=gunit.seqtkdir+"/seqtk";
        mkdir(tempdir);
